...
sudo apt install libserialport0 patchelf python3-serial git build-essential gdb cmake openjdk-17-jdk maven rust-all rust-src nodejs android-sdk-platform-tools python3-pip htop minicom mat2 bleachbit dconf-editor gnome-boxes tilp2 cura inkscape anki kiwix freecad arduino chromium codium signal-desktop xournalpp musescore3 -y
if $DATA_DRIVE
then
